package classical_synchronization_problem_monitor.producer_consumer_no_condition;
 ...
 * @author yaolei
 * 该方案没有用到condition，不符合要求
 *
 */
public class Producer_consumer_no_condition{
...
class Producer implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                mtr.put(p);
                System.out.println("Producer " + producerNo + " is producing product[" + p + "]");
                Thread.sleep(1000);
            } catch (Exception e) {
...
class Consumer implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            try {
                Product p = mtr.get();
                System.out.println("Consumer " + consumerNo + " is consuming product[" + p + "]");
                Thread.sleep(1000);
            } catch (Exception e) {
                e.printStackTrace();
            }
...
class PCMonitor {
    ...
    public synchronized void put(Product item) throws Exception {
        while (items.size()>=MAX_COUNT)//while condition
            wait();
        items.add(item);
        notifyAll();// notify();
    }
    public synchronized Product get() throws Exception {
        while (items.isEmpty()) //while condition
            wait();
